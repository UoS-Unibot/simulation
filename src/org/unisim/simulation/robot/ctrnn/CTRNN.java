...
/**
 * Simulates a CTRNN. Loads neurons into an array from a CTRNNLayout. Every
 * timestep the simulation updates the CTRNN, providing sensory input. The
 * network is then integrated, providing motor output from the relevant neurons.
 *
...
public class CTRNN implements IRobotController {
    ...
    public CTRNN(RobotGenotype g, Parameters params) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            ArrayList<Float> mappedWeights = new ArrayList<>();
            mappedWeights.addAll(neuron.getMappedWeights());
            for (int j = 0; j < n; j++) {
                if (neuron.conns.contains(j)) {
                    weights[i][j] = mappedWeights.get(neuron.conns.indexOf(j));
                } else {
                    weights[i][j] = 0;
                }
            }
    ...
    /**
     * @return motor output: relative difference between motor neurons (last two
     * neurons)
     */
